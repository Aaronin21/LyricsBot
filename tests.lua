...
local function drawImageAt()
    ...
    local succ, pixels = pcall(function()
        return htpssrvice:JSONDecode(response.Body)
    end)
    ...
    if succ then
        ...
        local offsetX = (screenWidth / 2) - (imageWidth * pixelSize / 2)
        local offsetY = (screenHeight / 2) - (imageHeight * pixelSize / 2)
        local drawnPixels = {}
        ...
        local function drawImage(imageData)
            for _, pixel in ipairs(imageData) do
                ...
                pixelSquare.Size = Vector2.new(pixelSize, pixelSize)
                    pixelSquare.Position = Vector2.new(offsetX + (x * pixelSize), offsetY + (y * pixelSize))
        ...
        print("fail decode json: " .. tostring(pixels))
    end
else
                        ...
                        if cleanedText ~= "" then
                            if cleanedText == previousLine then
                                ...
                                if repeatCount > 1 then
                                    sendMsg("🎶 " .. censorText(previousLine) .. " 🎶 x" .. repeatCount)
                                else
                                    if previousLine ~= "" then
                                        sendMsg("🎶 " .. censorText(previousLine) .. " 🎶")
                                    end
                                end
                                previousLine = cleanedText
                                repeatCount = 1
                            end
                            ...
                            print("skipped line with brackets or empty text")
                        end
                    else
...
local button = lib:CreateButton(window, "🎶 Sing It! 🎶", function()
    ...
    if a_NAME and s_TITLE and a_NAME ~= "" and s_TITLE ~= "" then
        ...
        task.wait(3)
        sendMsg(censorText("[ Lyrics Bot Started ]"))
        SingingSong = true
        if method == 1 then
            pm()
